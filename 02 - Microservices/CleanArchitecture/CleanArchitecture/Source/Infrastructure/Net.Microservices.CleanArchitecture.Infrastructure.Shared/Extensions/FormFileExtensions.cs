...
namespace Net.Microservices.CleanArchitecture.Infrastructure.Shared.Extensions
{
    public static class FileExtensions
    {
        ...
        public static byte[] ValidatePicture(byte[] pictureBinary, string mimeType)
        {
            ...
            return EncodeImage(image, mimeType);
        }

        public static byte[] EncodeImage(Image image, string mimeType)
        {
            using var memoryStream = new MemoryStream();
            image.Save(memoryStream, GetEncoder(mimeType));
            return memoryStream.ToArray();
        }

        private static IImageEncoder GetEncoder(string mimeType)
        {
            var format = TryFindFormatByMimeType(mimeType);
            if (format != null)
            {
                return Configuration.Default.ImageFormatsManager.GetEncoder(format);
            }

            throw new NotSupportedException($"Image format not supported for MIME type: {mimeType}");
        }

        private static IImageFormat TryFindFormatByMimeType(string mimeType)
        {
            Configuration.Default.ImageFormatsManager.TryFindFormatByMimeType(mimeType, out var format);
            return format;
        }
        ...
        private static byte[] EncodeImage<TPixel>(Image<TPixel> image, IImageFormat imageFormat)
